""""
  Library for the Maxim MAX30102 on CircuitPython

  Based on whilemind's MAX30100u library for Rasp Pi

  https: // github.com / whilemind / MAX30100u
  commit 56f4799d6111d7129e21ac5c906df5fc7d0c5026
...
MAX_I2C_ADDRESS = 0x57  # I2C address of the MAX30100 device
...
def _get_valid(d, value):
    try:
        ...
        raise KeyError("Value %s not valid, use one of: %s" %
                       (value, ', '.join([str(s) for s in d.keys()])))
...
class MAX30102():
    ...
    def _get_i2c(self):
        ...
        # Check to see if the device is the MAX30102..
        if (device_address != MAX_I2C_ADDRESS):
            raise error
            return
    ...
    def set_led_current(self, led_current_red=11.0, led_current_ir=11.0):
        ...
        self.i2c.write_byte_data(
            I2C_ADDRESS, LED_CONFIG, (led_current_red << 4) | led_current_ir)

    def set_mode(self, mode):
        reg = self.i2c.read_byte_data(I2C_ADDRESS, MODE_CONFIG)
        self.i2c.write_byte_data(
            I2C_ADDRESS, MODE_CONFIG, reg & 0x74)  # mask the SHDN bit
    ...
    def enable_interrupt(self, interrupt_type):
        self.i2c.write_byte_data(
            I2C_ADDRESS, INT_ENABLE, (interrupt_type + 1) << 4)
    ...
    def get_temperature(self):
        intg = _twos_complement(
            self.i2c.read_byte_data(I2C_ADDRESS, TEMP_INTG))
